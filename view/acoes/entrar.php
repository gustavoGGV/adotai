...
if (isset($_POST["input-numero"])) {
  ...
  $usuario = $usuarioController->encontrarUsuarioComTelefoneSenha(
    $_POST["input-numero"],
    $_POST["input-senha"],
  );
  ...
  if ($usuario instanceof PDOException) {
    $erro =
      "Erro de busca no banco de dados. Contate-nos: ajuda@adotai.com";
    if (AMBIENTE_DEV) {
      $erro = $usuario;
    }
  } elseif (!$usuario) {
    ...
    setcookie("idUsu", password_hash($usuario->getIdUsu(), PASSWORD_DEFAULT), [
    ...
    setcookie("telefoneUsu", $usuario->getTelefoneUsu(), [
      "expires" => time() + 60 * 60 * 24 * 120,
      "path" => "/",
      "domain" => "",
      "secure" => true,
