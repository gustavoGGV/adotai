...
if (isset($_POST["input-numero"])) {
    ...
    $dataNascimentoUsu = trim($_POST["input-data-nasc"])
        ? trim($_POST["input-data-nasc"])
        : null;
    $cepUsu = trim($_POST["input-cep"]) ? trim($_POST["input-cep"]) : null;
    $complementoUsu = trim($_POST["input-complemento"])
        ? trim($_POST["input-complemento"])
        : null;
    ...
    if (trim($_POST["input-senha-nova"])) {
        ...
        $inputSenhaAtualUsu = trim($_POST["input-senha-atual"])
            ? trim($_POST["input-senha-atual"])
            : "";

        $tamanhoSenhaNovaUsu = strlen(trim($_POST["input-senha-nova"]));
        $senhaNovaUsu = trim($_POST["input-senha-nova"])
            ? password_hash(trim($_POST["input-senha-nova"]), PASSWORD_DEFAULT)
            : null;
        $confirmacaoSenhaNovaUsu = password_verify(
            trim($_POST["input-confirmacao-senha-nova"]),
            $senhaNovaUsu,
        );
    }

    $telefoneUsu = trim($_POST["input-numero"])
        ? trim($_POST["input-numero"])
        : null;
    $tipoImagemPerfilUsu =
        !isset($_POST["input-imagem-gato"]) &&
        isset($_POST["input-imagem-cachorro"])
            ? "c"
            : "g";
    ...
    if ($alteracaoDeSenha) {
        ...
        $alteracoesUsuario = new Usuario(
            ...
            $senhaNovaUsu,
            $tamanhoSenhaNovaUsu,
            $confirmacaoSenhaNovaUsu,
            null,
            $tipoImagemPerfilUsu,
            false,
        );
        $invalidades = $usuarioController->alterarInformacoesUsuario(
            $alteracoesUsuario,
            true,
            $inputSenhaAtualUsu,
            $usuario->getSenhaUsu(),
