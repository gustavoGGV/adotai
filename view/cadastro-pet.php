...
<body class="d-flex flex-column min-vh-100">
  ...
  <div class="flex-fill">
    ...
    <form action="" method="post" class="container d-flex flex-column">
      <div class="mt-3 mb-3">
        <span>Nome *</span>
        <input value="<?= $cadastro && $cadastro->getNomePet()
            ? $cadastro->getNomePet()
            : null ?>" class="form-control mt-2" id="input-nome-pet" name="input-nome-pet" class="form-control" type="text" placeholder="nome do pet...">
      </div>
      <div class="mb-3">
        <span>Sexo *</span>
        <select class="mt-2 form-control" id="select-sexo-pet" name="select-sexo-pet">
          <option value="">escolha...</option>
          <option value="m" <?= $cadastro && $cadastro->getSexoPet() === "m"
              ? "selected"
              : null ?>>macho</option>
          <option value="f" <?= $cadastro && $cadastro->getSexoPet() === "f"
              ? "selected"
              : null ?>>fêmea</option>
        </select>
      </div>
        ...
        <select class="mt-2 form-control" id="select-especie-pet" name="select-especie-pet">
          ...
          <?php foreach ($especies as $especie): ?>
            <option value="<?= $especie->getIdEsp() ?>"
              <?= $cadastro &&
              $cadastro->getEspecie()->getIdEsp() === $especie->getIdEsp()
                  ? "selected"
                  : "" ?>>
      ...
      <div class="mb-3">
        ...
        <select class="mt-2 form-control" id="select-temperamento-pet" name="select-temperamento-pet">
          ...
          <?php foreach ($temperamentos as $temperamento): ?>
            <option value="<?= $temperamento->getIdTem() ?>" <?= $cadastro &&
$cadastro->getTemperamento()->getIdTem() === $temperamento->getIdTem()
    ? "selected"
    : null ?>><?= $temperamento->listarTemperamento() ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="mb-3">
        <span>Link de imagem *</span>
        <input value="<?= $cadastro && $cadastro->getLinkImagemPet()
            ? $cadastro->getLinkImagemPet()
            : null ?>" class="form-control mt-2" id="input-imagem-pet" name="input-imagem-pet" class="form-control" type="text" placeholder="link de uma imagem do pet...">
      </div>
      <div class="mb-3">
        <span>Descrição *</span>
        <input value="<?= $cadastro && $cadastro->getDescricaoPet()
            ? $cadastro->getDescricaoPet()
            : null ?>" class="form-control mt-2" id="input-descricao-pet" name="input-descricao-pet" class="form-control" type="text" placeholder="descrição do pet...">
      </div>
        <div id="erros" class="mt-3 text-danger p-2">
          <?= $mensagensDeInvalidade ?>
        </div>
      <div class="col-12 d-flex justify-content-between p-3">
        <button type="submit" class="mt-3 botao-cadastrar-pet btn col-5 text-white"><?= isset(
            $_GET["idPet"],
        )
            ? "Alterar"
            : "Cadastrar" ?></button>
        <button type="button" onclick="salvarPetAjax()" class="mt-3 botao-cadastrar-pet-ajax btn col-5 text-white"><?= isset(
            $_GET["idPet"],
        )
            ? "Alterar com AJAX"
            : "Cadastrar com AJAX" ?></button>
      </div>
    </form>
  </div>
